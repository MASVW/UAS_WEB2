<div x-data="{ price: false}"
  ...
  <div wire:poll.visible.15000ms class="mt-3 relative rounded-xl w-full overflow-y-scroll overflow-hidden h-custom-38-rem max-w-screen-md mx-auto">
    ...
    <button data-tippy-content="I'm a Tippy tooltip!" @if(!auth()->check()) disabled  @endif wire:click="addToChart({{$this->event->id}}, {{$price->id}})" class="transition hover:bg-sky-600 active:bg-sky-700 my-auto px-4 py-2 rounded-lg bg-sky-500 border text-white font-semibold uppercase">
        <div class="flex">
